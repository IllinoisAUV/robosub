...
int main(int argc, char **argv) {
  ...
  ros::Subscriber drpy_sub = nh.subscribe("/setpoint/angular/velocity", 1,
  ...
  ros::Subscriber dxyz_sub = nh.subscribe("/setpoint/linear/velocity", 1,
                                          &MotionController::SetdXYZ, controller);
  ros::Subscriber kill_sub = nh.subscribe("/arming", 10, &MotionController::arming, controller);
